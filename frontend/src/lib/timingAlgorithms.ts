...
function assignFromLine(holes: BlastHole[], settings: TimingSettings, line?: TimingLine) {
  ...
  const bandSize = Math.max(0.01, settings.rowTolerance);
  const ordered = [...holes].sort((a, b) => {
    const bandA = Math.floor(perpendicularDistance(a, activeLine) / bandSize);
    const bandB = Math.floor(perpendicularDistance(b, activeLine) / bandSize);
    return bandA - bandB || projectAlongLine(a, activeLine) - projectAlongLine(b, activeLine);
  });
  return ordered.map((hole, index) => {
    const bandIndex = Math.floor(perpendicularDistance(hole, activeLine) / bandSize);
    return {
      ...hole,
      rowIndex: bandIndex,
      columnIndex: index,
      firingOrder: settings.reverseOrder ? ordered.length - index : index + 1,
      timingGroup: `Band ${bandIndex + 1}`,
      delayMs: roundDelay(settings.startDelayMs + bandIndex * settings.rowDelayMs + index * settings.inRowDelayMs, settings),
    };
  });
}
